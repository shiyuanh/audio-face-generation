...
for iteration in range(1,opt.niter+1):
    ...
    errD = err_real - k*(err_fake + opt.res*torch.mean(torch.abs((real_recons-real).detach() - (fake_recons-fake))))
    errD.backward()
    optimizerD.step()
    ...
    errT = errG + opt.res * torch.mean(torch.abs(real_recons-real - (fake_recons-fake)))
    errT.backward()
    optimizerG.step()
    ...
    ########### Logging #########
    print('[%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_res: %.4f Measure: %.4f K: %.4f LR: %.8f'
              % (iteration, opt.niter, 
                 errD.data[0], errG.data[0], errT.data[0]-errG.data[0], measure, k, optimizerD.param_groups[0]['lr']))
    sys.stdout.flush()
    trainLogger.write('%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n' % \
                        (iteration, errD.data[0], errG.data[0], 0, measure, M_global.avg, k, balance, errT.data[0] - errG.data[0], 0))
    trainLogger.flush()
    ########### Learning Rate Decay #########
