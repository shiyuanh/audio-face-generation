...
from Data.dataset import VoxCeleb2, VoxCeleb3
import numpy as np
...
if opt.nz == 256:
    dataset = VoxCeleb2(loadSize=opt.loadSize,fineSize=opt.fineSize,flip=opt.flip, crop = opt.crop)
elif opt.nz == 1024:
    dataset = VoxCeleb3(loadSize=opt.loadSize,fineSize=opt.fineSize,flip=opt.flip, crop = opt.crop)
else:
    dataset = VoxCeleb2(loadSize=opt.loadSize,fineSize=opt.fineSize,flip=opt.flip, crop = opt.crop)
...
audio = torch.FloatTensor(opt.batchSize, opt.nz, 1, 1)
...
for iteration in range(start_epoch,opt.niter+1):
    ...
    - k * (err_fake + opt.res*torch.mean(torch.abs((real_recons-real).detach() - (fake_recons-fake)))) #+ \
           #0.5 * opt.metric * torch.mean(torch.abs((similarity(audio) - similarity(fea_fake)))))
    errD.backward()
    optimizerD.step()
